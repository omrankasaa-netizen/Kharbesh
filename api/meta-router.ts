...
// Meta Conversions API surface for Kharbesh.
//
...
export const metaRouter = createRouter({
  ...
  purchase: publicQuery.input(purchaseSchema).mutation(async ({ ctx, input }) => {
    ...
    try {
      const order = await getDb().query.orders.findFirst({ where: eq(orders.id, input.order_id) });
      ...
      const userData = buildUserData(
        {
          ...
          externalId: input.external_id ?? order.email,
