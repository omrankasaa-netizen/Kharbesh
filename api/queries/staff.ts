...
export async function upsertStaff(
  data: { email: string; name?: string; role: "staff" | "admin" | "super_admin" },
  actorUserId: number,
  actorEmail: string,
) {
  ...
  if (email === actorEmail.trim().toLowerCase() && data.role !== "super_admin") {
    throw new Error("You can't lower your own role. Ask another super admin to change it.");
  }
...
/** Revokes admin-panel access for an email and demotes their user row to 'user'. */
export async function removeStaff(email: string, actorUserId: number, actorEmail: string) {
  ...
  if (normalized === actorEmail.trim().toLowerCase()) {
    throw new Error("You can't remove your own staff access. Ask another super admin to do it.");
  }

  const db = getDb();

  await db.delete(staffRoles).where(eq(staffRoles.email, normalized));
  await db.update(users).set({ role: "user" }).where(eq(users.email, normalized));
