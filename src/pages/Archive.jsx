          ...
          {soldOut.map((p) => (
            <div key={p.id} className="kh-d-card opacity-70">
              <div className="kh-d-media">
                ...
                {p.images?.[0] ? <img src={p.images[0]} alt={p.name_en} loading="lazy" decoding="async" /> : <div className="flex items-center justify-center h-full text-muted-foreground p-4 text-center">{p.name_en}</div>}
              </div>
