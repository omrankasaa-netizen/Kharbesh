...
export default function Footer() {
  ...
  return (
    <footer className="kh-ink mt-0">
      <div className="max-w-[1400px] mx-auto px-4 sm:px-6 pt-14 pb-10">
        <div className="grid gap-10 md:grid-cols-4">
          <div className="md:col-span-2">
            <img src={BRAND_ASSETS.horizontalWhite} alt="Kharbesh" style={{ height: 30, width: 'auto', display: 'block' }} />
            <p className="mt-5 max-w-sm text-sm" style={{ color: muted }}>{t.footer.tagline}</p>
            <DotsMark lime className="mt-6" />
            <p className="mt-6 text-xs" style={{ color: 'rgba(251,246,235,.4)' }}>{t.footer.madeIn}</p>
          </div>
          <div>
        ...
        <img
          ...
          alt=""
          aria-hidden="true"
          className="w-full max-w-[520px] mx-auto mt-14 select-none"
          style={{ display: 'block', opacity: 0.92 }}
          loading="lazy"
        />

        <div className="mt-10 pt-6 flex flex-col sm:flex-row justify-between gap-2 text-xs" style={{ borderTop: `1px solid ${line}`, color: 'rgba(251,246,235,.4)' }}>
