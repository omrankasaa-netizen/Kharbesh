// Meta Pixel + Conversions API integration for Kharbesh.
//
// Implements Meta's recommended redundant setup end to end:
//   - Browser Pixel fires every core event (PageView on every page,
//     ViewContent, AddToCart, InitiateCheckout, Purchase) with a unique
//     event_id; the SAME event_id is posted to the server CAPI twin so Meta
...
//     the cookie banner stops it; the choice is remembered in localStorage.
//
// Everything is fail-safe: tracking must never throw into app code, and the
// whole module no-ops when VITE_META_PIXEL_ID is unset.

import { kh } from '@/api/khClient';

export const META_PIXEL_ID =
  (typeof import.meta !== 'undefined' && import.meta.env?.VITE_META_PIXEL_ID) || '';

const CONSENT_KEY = 'kharbesh-consent'; // 'granted' | 'denied'
const VID_KEY = '_kh_vid';
const AM_KEY = '_kh_am';
...
export function trackPurchasePixel({ eventId, value, currency = 'USD', items = [] }) {
...
export async function notifyPurchase({ orderId, eventId }) {
  if (!orderId || !hasMarketingConsent()) return;
  try {
    await kh.meta.purchase({
